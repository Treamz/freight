...
Future<List<Check>> runDoctor({
  ...
  return [
    ...await _checkTooling(tool),
    ..._checkConfiguration(projectRoot, configPath),
    ...await _checkIosProject(projectRoot, readPlist),
  ];
}
...
List<Check> _checkConfiguration(String projectRoot, String configPath) {
  ...
  if (!file.existsSync()) {
    return [
      Check.fail(
        ...
        fix: 'Declare your asset packs in freight.yaml. See doc/ios-setup.md.',
      ),
    ];
  }
  ...
  try {
    ...
    return [Check.fail('freight.yaml', detail: e.toString())];
  }
      ...
      if (plan.files.isEmpty) {
        checks.add(
          Check.fail(
            ...
            fix:
                'An empty pack packages and publishes cleanly, then fails in '
                'the app as a missing asset. Check the "files" globs.',
          ),
        );
      } else {
        checks.add(
